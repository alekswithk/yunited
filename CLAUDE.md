...
- `src/lib/content.js` loads the two JSON files and validates them against the Zod schemas in `src/lib/schema.js` at **build time**. A missing/misspelled field, a non-date, a bad RSVP URL or a duplicate event `id` fails `npm run build` with a message naming the file, index and field. `schema.js` is the authoritative description of the board's edit surface — keep it in lockstep with the JSON, and update it (not just the JSON) when the shape must change.
