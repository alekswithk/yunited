...
- `src/scripts/smooth-scroll.ts`, initialized by `BaseLayout.astro`, eases desktop wheel input. Its animation frame loop runs only while a wheel movement settles. Touch, keyboard, inputs, nested scroll areas and reduced-motion preferences keep native scrolling. Pointer or keyboard navigation cancels pending easing. This does not drive entrance animations, which remain CSS scroll timelines.
- The homepage's `.home-hero` reduces its phone padding after 3.2 seconds at the top of the page. Its page script cancels the timer on interaction, scrolling, navigation or visibility changes. Reduced-motion visitors get the compact layout immediately. This one-time padding transition changes layout to reveal the next section; it does not hide text or scroll the viewport.
...
A phased improvement plan exists at `~/.claude/plans/compare-this-static-website-cryptic-key.md`. The Astro migration was Phase 1. Remaining deferred work: the *recruiting-funnel* half of the partners/recruiting item (the `/partners` pitch page shipped) and further CSP/CI hardening. (Done: Phase 1 Astro migration, image optimization, generated sitemap with hreflang, Zod content schemas, the `/admin` panel — first Sveltia, now the first-party form + Worker that replaced it — all five languages (en/de/hr/bs/sr) live with event-content translation on save, and the buddy / kumstvo system on D1.) Check that plan before large structural changes so work aligns with the intended direction.
