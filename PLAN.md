...
| — | **Motion: the flag rule tracks the scroll** — the red/azure/gold band brackets the page top and bottom and slides with scroll position (`animation-timeline: scroll(root)`, mirrored directions so the two close towards each other, transform-driven, static under `prefers-reduced-motion` or without scroll-timeline support). Event cards reveal on scroll again: they carried a hard-coded `is-visible` since the Astro migration, which pre-revealed every one and meant the entrance never played |
