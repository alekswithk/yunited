...
| — | **Motion: the flag rule drifts** — the red/azure/gold band now brackets the page top and bottom and drifts sideways (mirrored directions, seamless one-tile loop, transform-driven, off under `prefers-reduced-motion`). Event cards reveal on scroll again: they carried a hard-coded `is-visible` since the Astro migration, which pre-revealed every one and meant the entrance never played |
