...
| — | **The TOC rail actually rendered in a browser — the first time — and three things were wrong** *(follow-up to the row above, which shipped unrendered)*. #55's rail claims were code comments no command in this repo can check, and one was false on its face: the comments sized the rail around a longest label of `"Buddy-System"` (12ch), but the real longest was `toc.buddy` in hr/bs/sr, **`"Sustav/Sistem prijatelja"` at 17ch**, on the very page that lists it. Measured in Chrome: Space Mono advances **612/1000 em**, so at `0.66rem` with `0.04em` tracking a character is 6.885px and that label is **117.0px** — against an 92px text box at the 1400px breakpoint, so it **wrapped to two lines from 1400px to ~1450px**, in three locales. No geometry fixes that: at 1440 the whole gutter is 154px. So the label was shortened to **"Prijatelji"** for hr/bs/sr — a TOC-specific short form, exactly as English already says "Buddy system" rather than the full heading. The pinned glossary term is untouched everywhere else and `validate.mjs` still matches it on the `prijat` stem (case-insensitive), so hr/bs/sr validate at **0 errors, 0 warnings**. Second: the `0.5rem` term in the `--toc-width` clamp **is** the rail's outer margin wherever the clamp's middle value is live, so the rail sat a constant **8px from the window edge across the whole 1400–1480 band** — raised to `1.5rem`, giving 14px at the breakpoint and 24px from 1440 up. Third: that also makes the `6.5rem` floor **live** (it binds 1400–~1424) where before it could never bind above the breakpoint and was dead code. Verified: no wrap at 1400 in hr/bs/sr, German `"Buddy-System"` (83px) fits the 86px box, the per-entry `is-on-dark` flip and `aria-current` both confirmed working over `#story`. **New rule in `CLAUDE.md`:** layout changes need a browser pass at the widths their breakpoints name, in a long-label locale, and a claim in a code comment is not verification |
